...
interface SignupProps {
  onSignupComplete: (userData: { email: string; name: string; location: string; leagues: LeagueInfo[] }) => void;
  onSwitchToLogin: () => void;
}

const Signup: React.FC<SignupProps> = ({ onSignupComplete, onSwitchToLogin }) => {
  ...
  // Fetch Cities when State/Region changes
  useEffect(() => {
  ...
  const validateLeagues = () => {
    ...
    selectedLeagues.forEach(leagueId => {
      ...
      if (values.every(v => v === '')) {
        newErrors[def.fields[0].id] = `Enter at least one rating for ${def.name}`;
      }
  ...
  const handleComplete = () => {
    ...
    const fullLocation = `${selectedCity}, ${selectedState}${isInternational ? `, ${selectedCountry}` : ''}`;
    onSignupComplete({ email, name, location: fullLocation, leagues });
  };
    ...
    switch (step) {
        ...
        return (
          <div className="w-full space-y-6 animate-fade-in">
            <div className="w-full text-center animate-slide-down">
              <h2 className="text-xl font-bold text-deep-charcoal dark:text-white mb-1">Start your journey</h2>
              <p className="text-sm text-muted-text dark:text-dark-text-muted">Join the world's best pool platform.</p>
            </div>
            <form onSubmit={handleNext} className="space-y-3 animate-slide-up stagger-item-1">
              <input 
              ...
              <div className="space-y-3">
                <input 
                  className={`input-field !py-3 !px-4 ${errors.confirmPassword ? 'border-red-500 bg-red-50 dark:bg-red-900/10' : ''}`} 
              ...
              <button type="submit" className="btn-primary !py-3 mt-2 shadow-lg active:scale-95 transition-all">
                Continue
              </button>
            </form>
          </div>
      ...
      case 2:
        return (
          <div className="w-full space-y-6 animate-fade-in">
            <div className="w-full text-center animate-slide-down">
              <h2 className="text-xl font-bold text-deep-charcoal dark:text-white mb-1">Where do you play?</h2>
              <p className="text-sm text-muted-text dark:text-dark-text-muted">Mandatory to find local clubs and events.</p>
            </div>
            ...
            <div className="space-y-5 animate-slide-up stagger-item-1">
              ...
              <div className="flex p-1 bg-gray-100 dark:bg-dark-surface rounded-2xl border border-soft-gray dark:border-dark-border">
                <button 
                  ...
                  className={`flex-1 py-2.5 text-xs font-black rounded-xl transition-all ${!isInternational ? 'bg-white dark:bg-dark-bg text-deep-charcoal dark:text-white shadow-sm' : 'text-muted-text dark:text-dark-text-muted'}`}
                >
                  United States
                </button>
                <button 
              ...
              <div className="space-y-3">
                ...
                {isInternational && (
                  <div className="relative">
                    <select 
                      className={`input-field !py-3 !px-4 appearance-none ${errors.location && !selectedCountry ? 'border-red-500' : ''}`}
                ...
                <div className="relative">
                  <select 
                    className={`input-field !py-3 !px-4 appearance-none ${errors.location && !selectedState ? 'border-red-500' : ''}`}
                    value={selectedState}
                    onChange={(e) => setSelectedState(e.target.value)}
                    disabled={loadingLoc || !selectedCountry}
                  >
                    <option value="" disabled>{loadingLoc ? 'Loading...' : isInternational ? 'Select Department / Province' : 'Select State'}</option>
                ...
                <div className="relative">
                  <input 
                    list="city-list"
                    className={`input-field !py-3 !px-4 ${errors.location && !selectedCity ? 'border-red-500' : ''}`}
                    placeholder={loadingLoc ? "Loading cities..." : "Search City"}
                    value={selectedCity}
              ...
              <div className="space-y-3 pt-2">
                <p className="text-[10px] font-black uppercase tracking-widest text-muted-text dark:text-dark-text-muted px-1">Are you in a league?</p>
                <div className="grid grid-cols-2 gap-2">
                  {AVAILABLE_LEAGUES.map((league, i) => (
                    <button
                      ...
                      className={`p-4 rounded-2xl border transition-all text-left animate-zoom-in stagger-item-${i+1} ${
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <span className="font-bold text-sm dark:text-white">{league.name}</span>
                        {selectedLeagues.includes(league.id) && (
                          <span className="material-symbols-outlined text-chalk-blue-dark text-sm animate-zoom-in">check_circle</span>
                        )}
                      </div>
                    </button>
                  ))}
              ...
              <div className="flex gap-3 pt-4">
                <button type="button" onClick={handleBack} className="flex-1 btn-secondary !py-3">Back</button>
                <button 
                  type="button"
                  onClick={selectedLeagues.length > 0 ? handleNext : handleComplete} 
      ...
      case 3:
        return (
          <div className="w-full space-y-6 animate-fade-in">
            <div className="w-full text-center animate-slide-down">
              <h2 className="text-xl font-bold text-deep-charcoal dark:text-white mb-1">League Rankings</h2>
              <p className="text-sm text-muted-text dark:text-dark-text-muted">Enter your current ratings. APA players can fill one or both formats.</p>
            </div>
            <div className="space-y-4 animate-slide-up stagger-item-1 max-h-[45vh] overflow-y-auto no-scrollbar pr-1 pb-4">
              {selectedLeagues.map((leagueId, i) => {
                ...
                return (
                  <div key={leagueId} className={`p-5 bg-white dark:bg-dark-surface border border-soft-gray dark:border-dark-border rounded-2xl animate-slide-left stagger-item-${i+1} shadow-sm`}>
                    <div className="flex items-center gap-2 mb-4">
                      <div className="w-2 h-2 rounded-full bg-chalk-blue animate-pulse"></div>
                      <h3 className="text-xs font-black uppercase tracking-widest text-deep-charcoal dark:text-white">{league.name}</h3>
                    </div>
                    
                    <div className={`${league.id === 'apa' ? 'space-y-5' : league.fields.length > 1 ? 'grid grid-cols-2 gap-3' : 'space-y-3'}`}>
                            ...
                            <>
                              ...
                              <div className="relative">
                                <input 
                                  ...
                                  className={`input-field !py-2.5 !px-4 !text-sm ${errors[field.id] ? 'border-red-500 bg-red-50 dark:bg-red-900/10' : ''}`}
                                  value={leagueRatings[field.id] || ''}
                                  onChange={(e) => handleRatingChange(field.id, e.target.value)}
                                />
                                {errors[field.id] && (
                                  <p className="text-[9px] text-red-500 font-bold mt-1 px-1 animate-slide-down">
                                    {errors[field.id]}
            ...
            <div className="flex gap-3 pt-2 animate-slide-up stagger-item-3">
              <button type="button" onClick={handleBack} className="flex-1 btn-secondary !py-3">Back</button>
              <button type="button" onClick={handleComplete} className="flex-[2] btn-primary !py-3 active:scale-95 transition-all shadow-chalk-blue/20">
                Complete Profile
              </button>
            </div>
          </div>
        ...
        <div className="w-full mt-auto mb-4 animate-slide-up stagger-item-3">
          <div className="relative mb-6">
            ...
            <div className="relative flex justify-center text-[10px] uppercase tracking-widest">
              <span className="bg-white dark:bg-dark-bg px-3 text-muted-text dark:text-dark-text-muted">Or sign up with</span>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <button type="button" className="btn-secondary !py-2.5 hover:bg-gray-50 transition-colors">
          ...
          <footer className="mt-6 text-center">
            <p className="text-xs text-muted-text dark:text-dark-text-muted">
              Already a member? 
              <button type="button" onClick={onSwitchToLogin} className="ml-1 text-chalk-blue-dark font-bold hover:underline transition-all active:scale-95 inline-block">Sign In</button>
            </p>
          </footer>
        </div>
      )}
